...
class HomeController extends Controller
{
    ...
    function welcome($id){
        $post = DB::select('select posts.id, posts.slug , posts.title,posts.full_img , posts.detail, posts.created_at, posts.meta, posts.description, posts.data, users.name from posts join users on users.id = posts.user_id where posts.user_id = ?', [$id]);

        // dd($post[0]->id);
        return view('welcome',['posts'=>$post]);
}

function blog_desc($id){
    $post = DB::select('select * from posts  join users on users.id = posts.user_id  where posts.slug = ?', [$id]);
    // dd($post[0]->title);
    return view('blog_desc',['posts'=>$post]);
}
    // All Categories
    ...
    function category(Request $request,$cat_slug,$cat_id){
        ...
        $welcome=Post::where('id',$cat_id)->orderBy('id','desc')->paginate(2);
        return view('category',['welcome'=>$welcome,'category'=>$category]);
    }
